<div class="fixed left-0 top-0 w-64 h-full bg-[#0F1035] p-4 overflow-y-auto">
    ...
    <ul class="mt-4">
        ...
        @if (auth()->user()->input_type == 'Group')
            <li>
                <a href="{{ route('target.department', 'department=' . $departmentID) }}"
                    class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
        ...
        @elseif (auth()->user()->input_type == 'Individual')
            <li>
                <a href="{{ route('target.department', 'employee=' . $userID) }}"
                    class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                    <i class="ri-crosshair-2-line text-2xl"></i>
                    <span class="ml-3">Input Target</span>
                </a>
        ...
        @if (auth()->user()->input_type == 'Group')
            <li>
                <a href="{{ route('actual.department', 'department=' . $departmentID) }}"
                    class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
        ...
        @elseif (auth()->user()->input_type == 'Individual')
            <li>
                <a href="{{ route('actual.department', 'employee=' . $userID) }}"
                    class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                    <i class="ri-book-2-line text-2xl"></i>
                    <span class="ml-3">Input Pencapaian Aktual</span>
                </a>
        ...
        <ul class="mt-2 space-y-2">
            ...
            @if (auth()->user()->input_type == 'Group')
                <li>
                    <a href="{{ route('report.index', 'department=' . $departmentID) }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
            ...
            @elseif (auth()->user()->input_type == 'Individual')
                <li>
                    <a href="{{ route('report.index', 'employee=' . $userID) }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                        <i class="ri-contacts-book-2-line text-2xl"></i>
                        <span class="ml-3">KPI Report {{ '(Employees)' }}</span>
                    </a>
            ...
            @if ($role == '' && $role == 'Inputer')
                <li>
                    <a href="{{ route('report.department', $departmentID . '?semester=&year=' . $currentYear) }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700" id="department-link">
            ...
            @if (auth()->user()->role == 'Approver' || $role == 'Mng Approver')
                ...
                <li>
                    <a href="{{ route('report-year.departmentTargetReport', 'year=' . $currentYear) }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
        ...
        <ul class="mt-2 space-y-2">
            <div class="flex items-center pb-0 border-b border-b-gray-600"></div>
            <li>
                <a href="{{ route('masterSupportingDocument') }}?department={{ $departmentID }}"
                    class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                    <i class="ri-database-line text-2xl"></i>
                    <span class="ml-3">Master Data Pendukung</span>
                </a>
            </li>
            <li>
                <a href="{{ route('supportingDocumentEmployee') }}?department={{ $departmentID }}"
                    class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                    <i class="ri-database-line text-2xl"></i>
        ...
        <ul x-show="open" x-transition:enter="transition ease-out duration-100"
            ...
            x-transition:leave-end="transform opacity-0 scale-95" class="mt-2 space-y-2">
            @if ($role == 'Approver' || $role == 'Mng Approver')
                <li>
                    <a href="{{ route('log-check.index', 'year=' . $currentYear . '&semester=' . $semester) }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
            ...
            @if ($role != '' && $role != 'Checker 1' && $role != 'Checker Factory' && $role != 'Checker WS')
                @if ($role == 'Checker Div 1' || $role == 'Checker Div 2')
                    <li>
                        <a href="{{ route('log-input.indexInput', 'department=' . $departmentID . '&month=' . $currentMonth . '&year=' . $currentYear) }}"
                            class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
            ...
            @if ($role == 'Approver' || $role == 'Mng Approver')
                <li>
                    <a href="{{ route('log-input.individual', 'department=' . $departmentID . '&month=' . $currentMonth . '&year=' . $currentYear) }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                ...
                <li>
                    <a href="{{ route('log-input.monitoringEmployee') }}?department=all&semester={{ $semester }}&year={{ $currentYear }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
                ...
                <li>
                    <a href="{{ route('log-input.monitoringDept') }}?semester={{ $semester }}&year={{ $currentYear }}"
                        class="flex items-center py-1.5 px-6 text-gray-300 hover:bg-gray-700">
